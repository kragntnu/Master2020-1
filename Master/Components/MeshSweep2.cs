...
namespace Master.Components
{
    public class MeshSweep2 : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            int nodeid = 1; //start with 1 to use in Abaqus
            for (int a = 0; a < (w + 1); a++)
            {
                for (int i = 0; i < (v + 1); i++)
                {
                    for (int j = 0; j < (u + 1); j++)
                    {
                        ...
                        nodenr.Add(p); //all nodes is one list (all nodes correctly listed: in u-direction,then v-direction, then w-direction). Here: should use node class? 
                        ...
                        Node node = new Node(nodeid, p); //all nodes added to node class
                        nodes.Add(node);
                        nodeid++;
                    }
                }
            ...
            DataTree<Point3d> elementpointstree = new DataTree<Point3d>();
            Point3d e1, e2, e3, e4, e5, e6, e7, e8;
            int elementindex = 0; //ok to start with 0 in grasshopper, but should start with 1 when used in Abaqus
            for (int a = 0; a < w; a++)
            {
                for (int i = 0; i < v; i++)
                {
                    for (int j = 0; j < u; j++)
                    {
                        e1 = nodetree.Branch(a, i)[j];
                        e2 = nodetree.Branch(a, i + 1)[j];
                        e3 = nodetree.Branch(a, i + 1)[j + 1];
                        e4 = nodetree.Branch(a, i)[j + 1];
                        e5 = nodetree.Branch(a + 1, i)[j];
                        e6 = nodetree.Branch(a + 1, i + 1)[j];
                        e7 = nodetree.Branch(a + 1, i + 1)[j + 1];
                        e8 = nodetree.Branch(a + 1, i)[j + 1];
                        elementpointstree.AddRange(new List<Point3d> { e1, e2, e3, e4, e5, e6, e7, e8 }, new GH_Path(elementindex)); //
                        elementindex++;
                    } //8 points collected in element branch with correct order. Have to connect it to the element class and connect it with nodes index.
            ...
            */
            ...
            //to make MeshGeometry to be viewed in Rhino 
            for (int a = 0; a < (w + 1); a++)
            {
                for (int i = 0; i < v; i++)
                {
